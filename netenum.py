...
def arguments():
    ...
    parser.add_argument('-w', '--write', metavar="<file>", dest="FILE", help="Save detected hosts in a file")
    parser.add_argument('-c', '--count', metavar="<int>", type=int, dest="COUNT", default=40, help="Maximum amount of packets to capture (default: 40)")
    parser.add_argument('-t', '--timeout', dest="TIMEOUT", metavar="<int>", default="3m", help="Maximum allowed scan time (default: 3 minutes)")
    parser.add_argument('-i', '--iface', dest="IFACE", metavar="<iface>", default="wlp3s0", help="Local interface to use during discovery (default: wlp3s0)")
    parser.add_argument('-cl', '--clear', dest='CLEAR', action='store_true', help="Clear screen before printing results")
...
def main():
    ...
    for pkt in arp_packets:
        hosts[pkt[ARP].psrc] = pkt[Ether].src if pkt[Ether].src != "ff:ff:ff:ff:ff:ff" else get_mac(pkt[ARP].psrc) 
